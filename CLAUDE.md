...
- **Stable browser fingerprint.** One current Chrome UA with matching `Sec-Fetch-*` / `Sec-CH-UA` / `Referer`. Do **not** add user-agent rotation — inconsistent identities are more anomalous than one consistent one.
